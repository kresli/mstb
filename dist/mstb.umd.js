(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('mobx'), require('mobx-state-tree')) :
  typeof define === 'function' && define.amd ? define(['exports', 'mobx', 'mobx-state-tree'], factory) :
  (global = global || self, factory(global.mstb = {}, global.mobx, global['mobx-state-tree']));
}(this, function (exports, mobx, mobxStateTree) { 'use strict';
  ...
  var require$$0 = getCjsExportFromNamespace(algorithms$1);
  ...
  /* istanbul ignore next */
  if (!cryptoBrowserify.pbkdf2Sync || cryptoBrowserify.pbkdf2Sync.toString().indexOf('keylen, digest') === -1) {
  ...
  */
  ...
  EncoderBuffer.prototype.join = function join(out, offset) {
    ...
    if (Array.isArray(this.value)) {
      ...
      else if (Buffer$4.isBuffer(this.value))
  ...
  inherits$2(DERNode, base$1.Node);
  ...
  inherits$2(DERNode$1, base$2.Node);
  ...
  function EVP_BytesToKey (password, salt, keyBits, ivLen) {
    if (!Buffer$9.isBuffer(password)) password = Buffer$9.from(password, 'binary');
    if (salt) {
      if (!Buffer$9.isBuffer(salt)) salt = Buffer$9.from(salt, 'binary');
      if (salt.length !== 8) throw new RangeError('salt should be Buffer with 8 byte length')
    }

    var keyLen = keyBits / 8;
    var key = Buffer$9.alloc(keyLen);
    var iv = Buffer$9.alloc(ivLen || 0);
    var tmp = Buffer$9.alloc(0);
  ...
  var fixProc = function (okey, password) {
    ...
    if (!match) {
      ...
      decrypted = new Buffer$a(match2[2].replace(/[\r\n]/g, ''), 'base64');
    } else {
      var suite = 'aes' + match[1];
      var iv = Buffer$a.from(match[2], 'hex');
      var cipherText = Buffer$a.from(match[3].replace(/[\r\n]/g, ''), 'base64');
  ...
  function parseKeys (buffer) {
    var password;
    if (typeof buffer === 'object' && !Buffer$b.isBuffer(buffer)) {
      password = buffer.passphrase;
      buffer = buffer.key;
    }
    if (typeof buffer === 'string') {
      buffer = Buffer$b.from(buffer);
    }
  ...
  function decrypt (data, password) {
    ...
    var key = pbkdf2$1.pbkdf2Sync(password, salt, iters, keylen, 'sha1');
  ...
  var mgf = function (seed, len) {
    var t = Buffer$c.alloc(0);
    var i = 0;
    var c;
    while (t.length < len) {
      c = i2ops(i++);
      t = Buffer$c.concat([t, createHash('sha1').update(seed).update(c).digest()]);
    }
    return t.slice(0, len)
  };

  function i2ops (c) {
    var out = Buffer$c.allocUnsafe(4);
  ...
  function withPublic (paddedMsg, key) {
    return Buffer$d.from(paddedMsg
  ...
  function oaep (key, msg) {
    ...
    var mLen = msg.length;
    var iHash = createHash('sha1').update(Buffer$e.alloc(0)).digest();
    var hLen = iHash.length;
    var hLen2 = 2 * hLen;
    if (mLen > k - hLen2 - 2) {
      throw new Error('message too long')
    }
    var ps = Buffer$e.alloc(k - mLen - hLen2 - 2);
    var dblen = k - hLen - 1;
    var seed = randombytes(hLen);
    var maskedDb = xor(Buffer$e.concat([iHash, ps, Buffer$e.alloc(1, 1), msg], dblen), mgf(seed, dblen));
    var maskedSeed = xor(seed, mgf(maskedDb, hLen));
    return new bn(Buffer$e.concat([Buffer$e.alloc(1), maskedSeed, maskedDb], k))
  }
  function pkcs1 (key, msg, reverse) {
    ...
    var ps;
    if (reverse) {
      ps = Buffer$e.alloc(k - mLen - 3, 0xff);
    } else {
      ps = nonZero(k - mLen - 3);
    }
    return new bn(Buffer$e.concat([Buffer$e.from([0, reverse ? 1 : 2]), ps, Buffer$e.alloc(1), msg], k))
  }
  ...
  var privateDecrypt = function privateDecrypt (privateKey, enc, reverse) {
    ...
    var zBuffer = Buffer$f.alloc(k - msg.length);
    msg = Buffer$f.concat([zBuffer, msg], k);
    if (padding === 4) {
  ...
  function oaep$1 (key, msg) {
    var k = key.modulus.byteLength();
    var iHash = createHash('sha1').update(Buffer$f.alloc(0)).digest();
    var hLen = iHash.length;
    if (msg[0] !== 0) {
      throw new Error('decryption error')
    }
  ...
  var rng = function nodeRNG() {
    return cryptoBrowserify.randomBytes(16);
  };
  ...
  function Controller(Props) {
    var uuid = Props.uuid ? Props.uuid : mobxStateTree.types.optional(mobxStateTree.types.identifier, uuid_1());
